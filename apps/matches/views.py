...
from .models import MatchModel, MatchScoreModel, LeagueSeason, PlayoffQualification
from .serializers import (
    MatchSerializer, MatchScoreSerializer, LeagueSeasonSerializer, 
    PlayoffQualificationSerializer
)
from .services import MatchSchedulerService
...
class UserTodayMatchesView(ListAPIView):
    """Get all matches for the logged-in user scheduled for today."""
    serializer_class = MatchSerializer
    permission_classes = [IsAuthenticated]
    ...
    def list(self, request, *args, **kwargs):
        ...
        tomorrow_utc = today_utc + timezone.timedelta(days=1)
        
        # Initial queryset within the date range
        base_queryset = MatchModel.objects.filter(
...
class MatchDetailView(APIView):
    """Get a specific match by league and match_day."""
    permission_classes = [IsAuthenticated]

    def get(self, request, league_id, match_day):
        try:
            match = MatchModel.objects.get(league_id=league_id, match_day=match_day)
        ...
        selections = match.selections.all().select_related('owner__user')
...
class LeagueStandingsView(APIView):
    ...
    def get(self, request, league_id):
        try:
            league = PrivateLeagueModel.objects.get(pk=league_id)
        ...
        standings = MatchSchedulerService.get_league_standings(league, match_type)
...
class LeagueSeasonView(APIView):
    """Get season information for a league."""
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, league_id):
        ...
        try:
            ...
            serializer = LeagueSeasonSerializer(season)
...
class PlayoffQualifiersView(ListAPIView):
    """Get playoff qualifiers for a league."""
    serializer_class = PlayoffQualificationSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        league_id = self.kwargs.get('league_id')
        return PlayoffQualification.objects.filter(league_id=league_id).order_by('regular_season_rank')
...
class LeagueWinnerView(APIView):
    """Get playoff winner of a league."""
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, league_id):
        ...
        winner = MatchSchedulerService.get_playoff_winner(league)
