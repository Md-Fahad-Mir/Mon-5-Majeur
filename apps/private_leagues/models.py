...
class PrivateLeagueModel(models.Model):
    """Represents a private fantasy league created by a team."""
    ...
    creator = models.ForeignKey(UserProfile, on_delete=models.CASCADE, blank=False, null=False, related_name='created_private_leagues')
    ...
    join_code = models.CharField(max_length=6, unique=True, blank=False, null=False)
    teams = models.ManyToManyField(UserProfile, related_name='joined_private_leagues', blank=True)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['creator', 'leauge_name'], name='unique_user_league_name')
        ]

    def save(self, *args, **kwargs):
        # Auto-generate join code if not set
        if not self.join_code:
            self.join_code = generate_invitation_code()
