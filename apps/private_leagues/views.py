...
from .serializers import PrivateLeagueSerializer, JoinPrivateLeagueSerializer, LeavePrivateLeagueSerializer, KickTeamSerializer, StartLeagueSerializer
...
class PrivateLeagueViewSet(ModelViewSet):
    serializer_class = PrivateLeagueSerializer
    permission_classes = [IsAuthenticated, IsOwner]

    def get_queryset(self):
        # Short-circuit when schema is being generated
        if getattr(self, "swagger_fake_view", False) or not self.request.user.is_authenticated:
            return PrivateLeagueModel.objects.none()
        
        user_profile = self.request.user.userprofile
        
        # If retrieving a specific league, allow if user is creator OR a member
        if self.action == 'retrieve':
            return PrivateLeagueModel.objects.filter(models.Q(creator=user_profile) | models.Q(teams=user_profile)).prefetch_related('matches').distinct()
        
        # Default: Only leagues created by logged-in user (for list, etc.)
        return PrivateLeagueModel.objects.filter(creator=user_profile).prefetch_related('matches')
    ...
    def active_leagues(self, request):
        ...
        active_leagues = (
        PrivateLeagueModel.objects.filter(
            is_active=True,
            is_started=False,
        )
        ...
        serializer = PrivateLeagueSerializer(active_leagues, many=True)
    ...
    @swagger_auto_schema(request_body=JoinPrivateLeagueSerializer)
    @action(
        detail=False,
        methods=['post'],
        permission_classes=[IsAuthenticated]
    ...
    def join(self, request):
        ...
        if not join_code:
            return Response({"detail": "Join code is required."}, status=400)

        try:
            league = PrivateLeagueModel.objects.get(join_code=join_code)
        except PrivateLeagueModel.DoesNotExist:
            return Response({"detail": "League with this join code not found."}, status=404)
    ...
    def my_leagues(self, request):
        ...
        leagues = PrivateLeagueModel.objects.filter(teams=user_profile).prefetch_related('matches')
        serializer = PrivateLeagueSerializer(leagues, many=True)
        return Response(serializer.data, status=200)



    # Leave a private league
    @swagger_auto_schema(request_body=LeavePrivateLeagueSerializer)
    @action(
        detail=False,
        methods=['post'],
        permission_classes=[IsAuthenticated]
    ...
    def leave(self, request, pk=None):
        try:
            league = PrivateLeagueModel.objects.get(pk=request.data.get('league_id'))
        except PrivateLeagueModel.DoesNotExist:
    ...
    @swagger_auto_schema(request_body=KickTeamSerializer)
    @action(
        detail=False,
        methods=['post'],   
        permission_classes=[IsAuthenticated, IsOwner]
    ...
    @swagger_auto_schema(request_body=StartLeagueSerializer)
    @action(
        detail=False,
        methods=['post'],
        permission_classes=[IsAuthenticated, IsOwner]
    ...
    def start_league(self, request, pk=None):
        ...
        try:
            ...
            MatchSchedulerService.initialize_season(league)
        ...
        try:
            MatchSchedulerService.generate_season_matches(league)
        except Exception as e:
